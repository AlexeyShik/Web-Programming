<template>
    <div id="app">
        ...
        <Middle :posts="posts" :users="users" :commentsSizes="commentsSizes" :comments="filterComments" :postId="postId"/>
        <Footer :users-size="usersSize" :posts-size="postsSize"/>
    </div>
</template>
    ...
    beforeCreate() {
        this.$root.$on("countCommentsSizes", () => {
          Object.values(this.posts).forEach(post => {
            this.commentsSizes[post.id] = this.commentsSizes[post.id] || {};
            this.commentsSizes[post.id].count = 0;
          })
          Object.values(this.comments).forEach(comment => {
            ++this.commentsSizes[comment.postId].count;
          })
        })

        this.$root.$on("setPostId", postId => {
          if (Object.values(this.posts).filter(p => p.id === postId).length !== 0) {
            this.postId = postId;
          }
        });
      ...
      this.$root.$on("onRegister", (login, name) => {
        ...
        if (login.length < 3 || login.length > 16) {
          this.$root.$emit("onRegisterValidationError", "Login should contain from 3 to 16 symbols");
          return;
        }
        ...
        if (name.length < 1 || name.length > 32) {
          this.$root.$emit("onRegisterValidationError", "Name should contain from 1 to 32 symbols");
          return;
        }
